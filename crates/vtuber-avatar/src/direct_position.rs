//! Position-aware upper-body solve owned by the application.
//
// Moved from the removed vendored bevy_vrm1 patch (see #91): the semantic
// head/body offsets are solved here into avatar-root translation and bounded
// torso lean on top of the direct-pose writer, against the unmodified
// upstream runtime.
...
use bevy_vrm1::prelude::{
    BodyTracking, ChestBoneEntity, HeadBoneEntity, NeckBoneEntity, RestGlobalTransform,
...
/// exists here.
#[must_use]
pub fn semantic_offset_to_model(semantic: Vec3, root_rest_rotation: Quat) -> Vec3 {
    root_rest_rotation * Vec3::new(semantic.x, semantic.y, -semantic.z)
}

fn clamp_vector_length(vector: Vec3, limit: f32) -> Vec3 {
    ...
    app.register_type::<BodyTrackingPositionInput>()
        ...
        .add_systems(
            ...
            apply_direct_body_position
                ...
                .after(crate::direct_pose::apply_direct_body_tracking)
...
/// [`apply_direct_body_tracking`](crate::direct_pose::apply_direct_body_tracking)
    ...
    for (root, input, profile, head, neck, upper_chest, chest, spine) in vrms.iter() {
        ...
        *root_global = match parent_global_before {
        ...
        if let Some(parent) = child_ofs.get(head.0).ok().map(ChildOf::parent)
            ...
            && let Ok((transform, mut global)) = transforms.get_mut(head.0)
        {
            *global = parent_global.mul_transform(*transform);
        }
    }
