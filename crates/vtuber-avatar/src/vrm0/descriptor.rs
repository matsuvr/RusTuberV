...
//! core model contract from the glTF JSON and reports the non-fatal
...
pub fn classify_legacy_shader(shader: &str) -> LegacyShaderKind {
    match shader {
        ...
        "VRM_USE_GLTFSHADER" | "Standard" | "UniGLTF/UniUnlit" => LegacyShaderKind::Passthrough,
    ...
    pub fn new(code: VrmCompatibilityWarningCode, context: impl Into<String>) -> Self {
        Self {
            ...
            context: context
                .into()
                .chars()
                .take(MAX_COMPATIBILITY_WARNING_CONTEXT)
                .collect(),
        }
    }
}
...
pub struct Vrm0MetaDiagnostics {
    ...
    /// Violent-usage permission field retained as a source string.
    pub violent_usage_name: Option<String>,
    /// Sexual-usage permission field retained as a source string.
    pub sexual_usage_name: Option<String>,
    /// Commercial-usage permission field retained as a source string.
    pub commercial_usage_name: Option<String>,
    /// Other-permission URL retained as a source string.
    pub other_permission_url: Option<String>,
    /// License names and both legacy URL spellings.
    pub license_name: Option<String>,
    /// License URL retained as a source string.
    pub license_url: Option<String>,
    /// Alternate license URL retained as a source string.
...
pub fn collect_legacy_compatibility_warnings(
    ...
    if let Some(properties) = legacy.get("materialProperties").and_then(Value::as_array) {
        ...
        for (index, property) in properties.iter().enumerate() {
            ...
            if let Some(textures) = property.get("textureProperties").and_then(Value::as_object) {
        ...
        for (group_index, group) in groups.iter().enumerate() {
            ...
            if let Some(raw_binds) = group.get("binds").and_then(Value::as_array) {
                for (bind_index, bind) in raw_binds.iter().enumerate() {
                    ...
                    let Some((mesh, morph)) = mesh.zip(morph) else {
                        continue;
                    };
...
fn legacy_curve_is_linear(curve: &[Value]) -> bool {
    ...
    keys.windows(2).all(|pair| {
        ...
        let outgoing = pair
            .first()
            .and_then(|key| key.get(3))
            .and_then(Value::as_f64);
        let incoming = pair
            .get(1)
            .and_then(|key| key.get(2))
            .and_then(Value::as_f64);
...
fn parse_vrm0_first_person(root: &Value, value: &Value) -> Result<VrmFirstPerson, VrmParseError> {
...
fn required_string(value: &Value, field: &str) -> Result<String, VrmParseError> {
    value
        .get(field)
        .and_then(Value::as_str)
        .map(String::from)
...
fn required_usize(value: &Value, field: &str, path: &str) -> Result<usize, VrmParseError> {
    value
        .get(field)
        .and_then(Value::as_u64)
        .and_then(|value| usize::try_from(value).ok())
...
fn required_f32(value: &Value, field: &str, path: &str) -> Result<f32, VrmParseError> {
    value
        .get(field)
        .and_then(Value::as_f64)
...
fn array3_object(value: &Value, path: &str) -> Result<[f32; 3], VrmParseError> {
    let object = value
        .as_object()
        .ok_or_else(|| VrmParseError::InvalidField {
            path: path.into(),
    ...
    for (slot, field) in result.iter_mut().zip(["x", "y", "z"]) {
        *slot = object
            .get(field)
            .and_then(Value::as_f64)
            .map(|value| value as f32)
...
fn validate_node_index(root: &Value, index: usize, path: &str) -> Result<usize, VrmParseError> {
    let count = root
        .get("nodes")
        .and_then(Value::as_array)
...
fn mesh_instance_nodes(root: &Value, mesh: usize, path: &str) -> Result<Vec<usize>, VrmParseError> {
    let meshes = root
        .get("meshes")
        .and_then(Value::as_array)
