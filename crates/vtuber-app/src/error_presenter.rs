...
pub fn present_error(
    ...
    let message = |ja: &'static str, en: &'static str, zh: &'static str, ko: &'static str| {
        language.pick(ja, en, zh, ko).to_string()
    };
    match error {
        ...
        OrchestratorError::InferenceFailed(_) => ErrorPresentation {
            ...
            user_message: message(
                ...
                "面部跟踪模型加载或运行失败。",
                "얼굴 트래킹 모델을 불러오거나 실행하는 데 실패했습니다.",
            ),
            suggested_actions: vec![UiAction::RetryAfterError, UiAction::DismissError],
        },
    }
...
mod tests {
    ...
    fn error_presenter_same_error_not_re_presented() {
        let mut presenter = ErrorPresenter::default();
        let error = OrchestratorError::NoCameraSelected;
        assert!(presenter.update(Some(&error), UiLanguage::Ja));
        assert!(!presenter.update(Some(&error), UiLanguage::Ja));
    }
    #[test]
    ...
    fn error_presenter_dismiss() {
        let mut presenter = ErrorPresenter::default();
        presenter.update(Some(&OrchestratorError::NoCameraSelected), UiLanguage::Ja);
        presenter.dismiss();
        assert!(presenter.current().is_none());
    }
    #[test]
    fn present_error_defaults_to_japanese_and_keeps_detail() {
        let presentation = present_error(
            &OrchestratorError::ImportFailed("bad file".to_owned()),
            UiLanguage::Ja,
        );
        ...
        assert!(
            presentation
                .user_message
                .starts_with("モデルを読み込めませんでした")
        );
        assert!(presentation.user_message.contains("bad file"));
        assert!(
            presentation
                .suggested_actions
    ...
    fn present_error_no_camera_has_refresh() {
        ...
        assert!(
            presentation
                .suggested_actions
                .contains(&UiAction::RefreshCameras)
        );
        assert!(
            presentation
                .user_message
