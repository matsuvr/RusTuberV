...
static JAPANESE_FONT_BYTES: &[u8] = include_bytes!(concat!(
    env!("CARGO_MANIFEST_DIR"), "/../../assets/fonts/LINESeedJP_A_TTF_Rg.ttf"
));
...
fn system_font_path(language: UiLanguage) -> io::Result<PathBuf> {
    #[cfg(target_os = "windows")]
    {
        ...
        let filename = match language {
            ...
            UiLanguage::Ja | UiLanguage::En => return Err(io::Error::new(io::ErrorKind::InvalidInput, "Japanese and English use the bundled font")),
        };
    ...
    {
        let _ = language;
        Err(io::Error::new(io::ErrorKind::Unsupported, "CJK system font loading supports Windows and macOS"))
    }
}

fn font_definitions(language: UiLanguage) -> io::Result<egui::FontDefinitions> {
    let mut fonts = egui::FontDefinitions::default();
    fonts.font_data.insert(JAPANESE_FONT_NAME.to_owned(), Arc::new(egui::FontData::from_static(JAPANESE_FONT_BYTES)));
    let locale_font = match language {
        ...
        UiLanguage::Zh | UiLanguage::Ko => {
            ...
            let data = std::fs::read(&path).map_err(|error| io::Error::new(error.kind(), format!("{}: {error}", path.display())))?;
            fonts.font_data.insert("locale-system-font".to_owned(), Arc::new(egui::FontData::from_owned(data)));
...
mod tests {
    ...
    fn japanese_and_english_need_no_system_font_or_network() {
        for language in [UiLanguage::Ja, UiLanguage::En] {
            ...
            assert_eq!(fonts.font_data[JAPANESE_FONT_NAME].font.as_ref(), JAPANESE_FONT_BYTES);
        }
    }
}
