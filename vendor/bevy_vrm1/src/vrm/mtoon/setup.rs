use crate::prelude::*;
use crate::vrm::expressions::{VrmMaterialBaseValues, VrmMaterialIndex};
...
fn turn_to_mtoon_material(
    ...
    added_materials.iter().for_each(|(entity, handle)| {
            ...
            if let Some(mut material) = standard_materials.get_mut(handle.id()) {
                ...
                if let Some(index) = index {
                    commands.entity(entity).insert((
                        ...
                        VrmMaterialBaseValues::from_standard(&material),
                    ));
                }
            }
            return;
        ...
        if let Some(index) = index {
            cmd.insert((VrmMaterialIndex(index), base_values));
        }
    });
}
